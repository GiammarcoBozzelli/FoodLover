...
def try_parse_int(num):
    """
    Try to parse a strinag in an intger; return True if parsing works
    """
    try:
        int(num)
        return True
    ...
    if not has_numbers(user_in := input("Type food name: ")):
        ...
        while not stop:
            ...
            if not has_numbers(user_in := input("Type food name: ")):
                ...
                print("\n", "You are allowed only to use letters for name," +
                      'TRY AGAIN!', "\n")
...
def number_input(nutvalues, is_float):
    ...
    if is_float:
        ...
        if try_parse_float(user_in := input(f"Type food {nutvalues}: ")):
            ...
            print("\n", f"You are allowed only to use floats for {nutvalues},"
                  + "TRY AGAIN!", "\n")
            while not stop:
                # loop for input only correct digits
                if try_parse_float(user_in :=
                                   input(f"Type food {nutvalues}: ")):
                    stop = True
                else:
                    print("\n", f"You are allowed only to use floats" +
                          "for {nutvalues}," + "TRY AGAIN!", "\n")
        ...
        if try_parse_int(user_in := input(f"Type food {nutvalues}: ")):
            ...
            # property contains illegal character(s)
            print("\n", f"You are allowed only to use integers"
                  + "for {nutvalues}, TRY AGAIN!", "\n")
            while not stop:
                # loop for input only correct digits
                if try_parse_int(user_in :=
                                 input(f"Type food {nutvalues}: "), is_float):
                    stop = True
                else:
                    print("\n", f"You are allowed only to use integers"
                          + "for {nutvalues}, TRY AGAIN!", "\n")
...
def update_dataframe(df):
    """
    Collect user input using the functions above and adding it to the CSV file
    """
    food_properties = dict()
    print("Welcome, here you can add a food to the database...")
    user_choice = input("WARNING! Quit is allowed only here, "
                        + "do you really want to continue? [[Y]/n] ").strip()

    if user_choice == "Y" or user_choice == "y" or user_choice == "":
        # user decides to modify CSV file
        food_properties["name"] = ["" + str(name_input()) + ""]
        food_properties["carbohydrate"] = [str(number_input("carbs",
                                               is_float=True)) + "g"]
        food_properties["total_fat"] = [str(number_input("fats",
                                            is_float=True)) + "g"]
        food_properties["protein"] = [str(number_input("proteins",
                                          is_float=True)) + " g"]
        food_properties["calories"] = [int(number_input("calories",
                                           is_float=False))]
...
def read_dataframe(file_name):
    ...
    # modify csv columns for usability
...
def get_info(arguments):
    '''
    Search for the correct output to return according the
    user input searching on the csv file
    '''
    df = read_dataframe('nutrition.csv')
    # collect the row corrisponding to the input food
    row = df.loc[df['name'] == arguments.food]
    output = ''
    # checks which arguments have been passed
    ...
    if (not arguments.add and not arguments.protein and not arguments.carbs
       and not arguments.fats and not arguments.calories
       and not arguments.show):
        output += "protein: " + str(row["protein"].values[0]) + "\n"
        output += "calories: " + str(row["calories"].values[0]) + ' kcal\n'
        output += "carbs: " + str(row["carbohydrate"].values[0]) + "\n"
        output += "fats: " + str(row["total_fat"].values[0]) + "\n"
...
def parse_arguments():
    ...
    parser.add_argument('-p', '--protein', action='store_true',
                        help='Displays' + ' how many grams of proteins'
                             + ' the food has per 100g')
    parser.add_argument('-cal', '--calories', action='store_true',
                        help='Displays how many calories'
                             + ' the food has per 100g')
    parser.add_argument('-f', '--fats', action='store_true',
                        help='Displays how many grams of fats'
                             + ' the food has per 100 grams')
    parser.add_argument('-c', '--carbs', action='store_true',
                        help='Displays how many grams of '
                             + 'carbohydrates the food has per 100g')
...
def check_input(args):
    ...
    '''
    # handles eventuals food names wiht upper characters
    # or made of more than one word (e.g 'Nuts pecans')
    food_name = ' '.join(args.food).lower()
    df = read_dataframe('nutrition.csv')

    # checks which arguments are being passed and redirects the funcion
    if args.show:
        show_food(df)
        quit()
    elif args.food == []:
    ...
    # checks if the food name contains only valid characters
...
def show_food(df):
    ...
    '''
    # collects the starting letters of the food the user is looking for
    letter = input('Type the starting letters of the food'
                   + ' you are looking for: \n')
    out = df[df['name'].str.startswith(letter)]
